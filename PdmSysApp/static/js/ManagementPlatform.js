simulateFloodlightSensor = function () {
    ...
    $.get("/floodlightMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            ...
            newSensor.luminance = Math.round(500*Math.random()*100)/100; //有效数字精确到小数点后两位
...
simulateTemperatureSensor = function () {
    ...
    $.get("/temperatureMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            newSensor = workingSensors[i];
            newSensor.temperature = Math.round(500*Math.random()*100)/100; //有效数字精确到小数点后两位
...
simulateHumiditySensor = function () {
    ...
    $.get("/humidityMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            newSensor = workingSensors[i];
            newSensor.humidity = Math.round(500*Math.random()*100)/100; //有效数字精确到小数点后两位
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.allSensorsJSON.length; i++) {
                ...
                if (data.allSensorsJSON[i].luminance < 5.00)
                    alert("设备：【" + data.allSensorsJSON[i].name + "】 剩余工作周期不足！" + "\n为：" + data.allSensorsJSON[i].luminance + "需要维护！");
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].luminance + "</td>" +
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.allSensorsJSON.length; i++) {
                ...
                if (data.allSensorsJSON[i].temperature < 5.00)
                    alert("设备：【" + data.allSensorsJSON[i].name + "】 剩余工作周期不足！" + "\n为：" + data.allSensorsJSON[i].temperature + "需要维护！");
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].name + "</td>" +
                    "<td style='text-align:center'>" + deviceStatusString + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].temperature + "</td>" +
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.allSensorsJSON.length; i++) {
                ...
                if (data.allSensorsJSON[i].humidity < 5.00)
                    alert("设备：【" + data.allSensorsJSON[i].name + "】 剩余工作周期不足！" + "\n为：" + data.allSensorsJSON[i].humidity + "需要维护！");
...
$(document).ready(function () {
    ...
    $("#floodlightShowBtn").click(function () {
        ...
        floSim = window.setInterval(simulateFloodlightSensor, 25000);
        floRel = window.setInterval(reloadFloodlightTable, 20000);
    });
     ...
     $("#temperatureShowBtn").click(function () {
        ...
        temSim = window.setInterval(simulateTemperatureSensor, 25000);
        temRel = window.setInterval(reloadTemperatureTable, 20000);
    });
    ...
    $("#humidityShowBtn").click(function () {
       ...
       if ("floodlight" == showType){
        ...
        showType = "humidity";
        reloadHumidityTable();
        humSim = window.setInterval(simulateHumiditySensor, 25000);
        humRel = window.setInterval(reloadHumidityTable, 20000);
    });
    ...
    $("#addSensor").click( function () {
        if ("null" == showType){
            ...
            // 设置title
            if ("floodlight" == showType)
                $('.modal-title').text('新建AGV小车');
            else if ("temperature" == showType)
                $('.modal-title').text('新建自动化立体库');
            else if ("humidity" == showType)
                $('.modal-title').text('新建叉车');
        }
    ...
    $("#monitorTable").on('click', ".btnEdit", function () {
        ...
        if ("工作" == $(this).parent("td").prev().prev().text()) {
            ...
            $("#btnControl").text("派遣维护人员");
        }else{
            deviceStatus = "False";
            $("#btnControl").attr("class", "btn btn-primary");
            $("#btnControl").text("完成维护");
        }
